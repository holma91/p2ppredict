...
const priceFetcher = async ({ queryKey }: any) => {
	...
	const url = `https://api.coingecko.com/api/v3/simple/price?ids=${assetsString}&vs_currencies=usd`;
	console.log(url);
...
export const useFetchPrices = (assets: string[]) => {
	const { isLoading, isError, data } = useQuery(['prices', { assets }], priceFetcher, {
		staleTime: Infinity,
	});
